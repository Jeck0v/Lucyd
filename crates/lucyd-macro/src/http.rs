//! Implementation of the `#[lucyd_http(...)]` attribute macro.
//!
...
/// Parsed arguments for the `#[lucyd_http(...)]` attribute.
pub struct HttpArgs {
    ...
    pub tags: Vec<String>,
    /// Optional query-string type for JSON Schema generation.
...
/// Accumulator for `#[lucyd_http(...)]` arguments as they're parsed.
///
...
impl Parse for HttpArgs {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        // String arguments use `"value"`, while `query`, `request` and
        // `response` accept a bare type path (e.g. `MyStruct`).
            ...
            match key.to_string().as_str() {
                ...
                _ => {
                    return Err(common::unknown_argument_error(
                        &key,
                        "method, path, description, tags, query, request, response",
                    ));
                }
            }
...
/// Expands the `#[lucyd_http(...)]` attribute.
///
...
pub fn expand(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        ::lucyd::_private::inventory::submit! {
            ::lucyd::_private::lucyd_types::endpoint::EndpointMetaStatic {
                name:              #fn_name,
                path:              #path,
                protocol:          ::lucyd::_private::lucyd_types::endpoint::Protocol::Http,
