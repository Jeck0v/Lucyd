...
//! and is embedded into the `lucyd-core` binary at compile time via
...
///
/// The folder path is relative to the crate root (`lucyd-core/`).
/// Run `cargo xtask build-ui` before building `lucyd-core` in a
